const thankYouLetter = ({ responses, tone, content_model_name }) => {
  const output = `You're assisting a fundraiser who works for a non-profit organization called ${responses.organization_name}. 
        ...
        The tone of the ${content_model_name} should be ${tone}.
...
const contentRewriter = ({ responses, tone }) => {
  const output = `Rewrite the following content to be more ${tone}. Every version should be unique:
